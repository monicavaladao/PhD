...
function this = fit( this, samples, values )
    ...
    if ischar( this.regressionFcn )
        ...
        % Filter terms of the regression function by sum of degrees
        if ~isempty(dj)
            if this.options.filterBySumDegrees
                maxSumDegrees = max(dj);
                this.regressionFcn = this.regressionFcn(sum(this.regressionFcn, 2) <= maxSumDegrees, :);
            end
        end
